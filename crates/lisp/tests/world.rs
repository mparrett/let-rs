...
/// A MacroVm plus the spells namespace. Spell source uses `thread` and
/// the ctx helpers, which the pack keeps private (ADR-042), so it has to
/// evaluate inside the pack.
struct Lab {
    vm: MacroVm,
    ns: Rc<Namespace>,
}
...
fn world_apply_clamps_area_to_grid() {
    ...
    let painted = vm.eval(src).expect("world-apply!");
...
fn world_set_tile_rejects_out_of_range_coord() {
    ...
    let mut vm = vm_with_world(7, 5);
    let r = vm.eval("(world-set-tile! 5000000000 0 'fire)");
    assert!(
...
fn vm_with_world_handle(w: u32, h: u32) -> (Lab, Rc<RefCell<World>>) {
    let world = Rc::new(RefCell::new(World::new(w, h).expect("dims fit")));
...
fn world_tick_decays_over_time() {
    ...
    .unwrap();
    let r = vm.eval("(world-tick!)").unwrap();
