...
fn single_plain_rune() {
    assert_eq!(tape_to_sexpr("☲").unwrap(), "(list fire)");
    assert_eq!(tape_to_sexpr("☵").unwrap(), "(list ice)");
}

#[test]
fn canonical_example() {
    assert_eq!(
        tape_to_sexpr("☲ ☴ 3 ☵").unwrap(),
...
fn multi_digit_numeral() {
    assert_eq!(tape_to_sexpr("☰ 42").unwrap(), "(list (power 42))");
}
...
fn oversized_number_errors_not_panics() {
    ...
    let r = tape_to_sexpr("☴ 99999999999999999999");
...
fn whitespace_between_runes_is_optional() {
    ...
    assert_eq!(tape_to_sexpr("☲☵☳").unwrap(), "(list fire ice bolt)");
}

#[test]
fn earth_rune_lexes() {
    // ☷ KUN (earth) — third base element. Plain rune, no numeric arg,
    // pairs with fire/ice via the prelude's mix table (ADR-030).
    assert_eq!(tape_to_sexpr("☷").unwrap(), "(list earth)");
    assert_eq!(tape_to_sexpr("☲ ☵ ☷").unwrap(), "(list fire ice earth)");
}

#[test]
fn aftershock_rune_pairs_with_number() {
    // ☱ DUI (lake) — aftershock. Parametrized like area/power; the immediately
    // following numeral becomes its arg (ticks until the delayed
    // re-cast fires).
    assert_eq!(tape_to_sexpr("☱ 7").unwrap(), "(list (aftershock 7))");
    assert_eq!(
        tape_to_sexpr("☲ ☱ 3 ☴ 1").unwrap(),
